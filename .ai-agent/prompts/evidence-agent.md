...
- Read `.ai-agent/evidence/{{ADU_ID}}.json`. Do not replace its schema or overwrite it directly. The canonical `.ai-agent/evidence/{{ADU_ID}}.json` is generated by the runtime compiler.
- Write only `.ai-agent/evidence/{{ADU_ID}}-notes.md` when explanatory notes are needed.
...
```json
{
  ...
  "changed_files": [".ai-agent/evidence/{{ADU_ID}}-notes.md"],
  "commands_run": [],
  "artifacts": [".ai-agent/evidence/{{ADU_ID}}-notes.md"],
  "risks": [],
  "next_agent": null
}
```
