...
**数据来源:** 公司 5G 产品功能清单及控标点清单的 `功能细分` 列  
**规范参考日期:** 2026-05-23  
**规范锚点:** 以 3GPP Release 19 为稳定测试锚点，兼顾 Release 20 增强项  
**测试对象:** 公司 5GC 核心网产品、公司 5GC 网管平台、UE/gNB 模拟器或测试仪表、5G-LAN、专用 RAN/UE/IMS/安全/性能环境  
...
本手册面向公司内部测试和研发人员，用于公司现有 5G 产品功能项的验收、回归和缺陷复现，按功能域组织测试内容，避免按来源材料或历史分册拆分。每个用例均给出规范依据、前置条件、正常流程、异常流程、预期结果和证据要求，可用于实验室测试、专网专项验收、版本回归、研发自测和问题定位。
...
| 组件 | 用途 |
| --- | --- |
| 公司 5GC 核心网 NF（NRF/AMF/SMF/UPF/AUSF/UDM/UDR/PCF/NSSF） | 公司 5GC 被测系统 |
| 公司 5GC 网管平台 | 用户、DNN、切片、策略、安全与运维配置面 |
| 产品数据库 | 订阅数据、策略与测试数据存储 |
| UE/gNB 模拟器或测试仪表 | gNB/UE、注册、PDU Session、切片和 NAS 流程模拟 |
| 5G-LAN 自动化测试套件 | Ethernet PDU、5G-LAN L2/L3、UPF HA 相关测试资产 |
| tcpdump/tshark/scapy | N2/NAS/N4/N3/N6/以太网帧抓包与断言 |
| curl/jq | SBI、网管平台 API、Prometheus/metrics 验证 |
...
| 功能域 | 用例编号 | 用例名称 | 对应功能细分 | 环境类型 |
| --- | --- | --- | --- | --- |
| 接入与移动性 | TC-REG-001 | 初始注册 | 初始注册 | 通用环境 |
| 接入与移动性 | TC-REG-002 | 去注册 | 去注册 | 通用环境 |
| 接入与移动性 | TC-REG-003 | TAI List 与移动性限制 | TAI List 与移动性限制 | 通用环境 |
| 接入与移动性 | TC-CONN-001 | CM 状态转换与 NAS 连接 | CM 状态转换与 NAS 连接 | 通用环境 |
| 接入与移动性 | TC-SR-001 | Service Request | Service Request | 通用环境 |
| 接入与移动性 | TC-PAGE-001 | 下行数据寻呼 | 下行数据寻呼 | 通用环境 |
| 接入与移动性 | SE-HO-001 | Xn 切换 | Xn 切换 | 专用环境 |
| 接入与移动性 | SE-HO-002 | N2 切换 | N2 切换 | 专用环境 |
| 接入与移动性 | SE-HO-003 | 4G/5G 互操作 | 4G/5G 互操作 | 专用环境 |
| 接入与移动性 | CC-AMF-001 | AMF 负载均衡、重定向与隐式去注册 | AMF Load Re-balancing、NRF 引导到备用 AMF、网络侧隐式去注册 | 通用环境 |
| 接入与移动性 | CC-AM-001 | AM Policy Association 与移动性策略下发 | 注册过程中建立 AM 策略关联，下发服务区域限制等移动性策略 | 通用环境 |
| 会话管理、用户面与 PFCP | TC-PDU-001 | PDU Session 建立、修改、释放 | PDU Session 建立、修改、释放 | 通用环境 |
| 会话管理、用户面与 PFCP | TC-PDU-002 | IPv4/IPv6/IPv4v6 会话类型 | IPv4/IPv6/IPv4v6 会话类型 | 通用环境 |
| 会话管理、用户面与 PFCP | TC-PDU-003 | Ethernet PDU Session | Ethernet PDU Session | 通用环境 |
| 会话管理、用户面与 PFCP | TC-UP-001 | N3/N6 数据转发 | N3/N6 数据转发 | 通用环境 |
| 会话管理、用户面与 PFCP | TC-NAT-001 | N6 NAT | N6 NAT | 通用环境 |
| 会话管理、用户面与 PFCP | TC-PFCP-001 | PFCP Association 与心跳 | PFCP Association 与心跳 | 通用环境 |
| 会话管理、用户面与 PFCP | TC-PFCP-002 | PDR/FAR/QER 规则下发 | PDR/FAR/QER 规则下发 | 通用环境 |
| 会话管理、用户面与 PFCP | CC-PDU-001 | SSC Mode 1 IP 保持专项 | SSC Mode 1，会话释放/重建后 IP 地址保留 | 通用环境 |
| 会话管理、用户面与 PFCP | CC-PDU-002 | SSC Mode 2 / LADN 区域会话专项 | SSC Mode 2、LADN 区域绑定、离开区域触发会话释放、重入区域重建 | 通用环境 |
| 会话管理、用户面与 PFCP | CC-PDU-003 | SSC Mode 3 / MA PDU Session 专项 | SSC Mode 3 多会话迁移、MA PDU Session 多接入连续性 | 专用环境 |
| 会话管理、用户面与 PFCP | CC-DDN-001 | IDLE 下行缓存与 N4 下行数据通知专项 | IDLE 态下行数据缓存、UPF 向 SMF 发送下行数据通知 | 通用环境 |
| 会话管理、用户面与 PFCP | CC-ENDM-001 | End Marker 生成和转发专项 | UPF 或 SMF 生成 End Marker | 通用环境 |
| 会话管理、用户面与 PFCP | CC-UPF-001 | N9 多 UPF / UL CL 分流专项 | N9 接口、多 UPF 部署、UL CL、Branching Point、本地分流 | 专用环境 |
| 会话管理、用户面与 PFCP | CC-URR-001 | PFCP URR 用量上报专项 | PDR/URR 上报、时间阈值和字节阈值 Usage Report | 通用环境 |
| 策略、计费、切片与能力开放 | TC-QOS-001 | QoS 与包过滤 | QoS 与包过滤 | 通用环境 |
| 策略、计费、切片与能力开放 | TC-SLICE-001 | 切片选择与 DNN 绑定 | 切片选择与 DNN 绑定 | 通用环境 |
| 策略、计费、切片与能力开放 | TC-SLICE-002 | 切片准入控制 | 切片准入控制 | 通用环境 |
| 策略、计费、切片与能力开放 | CC-PCF-001 | SM Policy Association 与动态 PCC 推送 | Npcf_SMPolicyControl 创建、更新、删除，PCF 主动更新 PCC 规则 | 通用环境 |
| 策略、计费、切片与能力开放 | CC-CHG-001 | Nchf 计费接口与 CDR 专项 | Nchf_ConvergedCharging、离线 CDR、用量统计和会话终止计费 | 通用环境 |
| 策略、计费、切片与能力开放 | CC-AF-001 | AF/NEF 流量影响 Traffic Influence | AF 经 NEF/PCF 请求流量引导到指定 UPF/DN | 通用环境 |
| 策略、计费、切片与能力开放 | CC-OAM-001 | UE 事件订阅、外部 UE 信息输入、外部 QoS 请求 | 监控 UE 特定事件、允许外部方提供 UE 信息、基于外部请求处理 QoS | 通用环境 |
| 5G-LAN、工业专网与高可靠 | TC-5GLAN-001 | 5G LAN 二层组网 | 5G LAN 二层组网 | 通用环境 |
| 5G-LAN、工业专网与高可靠 | TC-5GLAN-002 | VLAN 透传 | VLAN 透传 | 通用环境 |
| 5G-LAN、工业专网与高可靠 | TC-5GLAN-003 | 二层三层互通与后路由 | 二层三层互通与后路由 | 通用环境 |
| 5G-LAN、工业专网与高可靠 | TC-HA-001 | 5G LAN 用户面 HA | 5G LAN 用户面 HA | 通用环境 |
| 5G-LAN、工业专网与高可靠 | TC-DEV-001 | 下挂设备发现 | 下挂设备发现 | 通用环境 |
| 5G-LAN、工业专网与高可靠 | SE-HA-001 | AMF Set/MME Pool 冗余与负载分担 | AMF Set/MME Pool 冗余与负载分担 | 专用环境 |
| 5G-LAN、工业专网与高可靠 | SE-HA-002 | UPF 主备切换 | UPF 主备切换 | 专用环境 |
| 5G-LAN、工业专网与高可靠 | SE-HA-003 | 5G-LAN MAC/组播状态同步 | 5G-LAN MAC/组播状态同步 | 专用环境 |
| 5G-LAN、工业专网与高可靠 | SE-RED-001 | 单基站双发选收 | 单基站双发选收 | 专用环境 |
| 5G-LAN、工业专网与高可靠 | SE-RED-002 | 双基站双发选收 | 双基站双发选收 | 专用环境 |
| 5G-LAN、工业专网与高可靠 | SE-RED-003 | 20ms/99.999% 可靠性指标 | 20ms/99.999% 可靠性指标 | 专用环境 |
| 5G-LAN、工业专网与高可靠 | CC-MCAST-001 | IPv4/IPv6 组播、IGMP/MLD 专项 | IPv4 组播、IPv6 组播、IGMP/MLD 加入/离开 | 专用环境 |
| 5G-LAN、工业专网与高可靠 | CC-VLAN-001 | 802.1Q VLAN 透传专项 | Ethernet PDU Session 内 802.1Q VLAN Tag 透明传输、N6 VLAN 子接口 | 专用环境 |
| 5G-LAN、工业专网与高可靠 | CC-L2ROBUST-001 | MAC 老化、广播风暴抑制、环路检测 | MAC aging、广播/未知单播风暴抑制、L2 环路检测 | 专用环境 |
| 5G-LAN、工业专网与高可靠 | CC-RAN-001 | 切片 RB 资源预留/动态共享 | NSSAI 绑定 DNN、DNN 下 QoS 规则、基站 RB 资源静态预留/动态共享 | 专用环境 |
| 5G-LAN、工业专网与高可靠 | SE-TSN-001 | TSN / IEEE 802.1AS 工业时间同步 | TSN 时间同步、IEEE 802.1AS/gPTP、5G 时间同步承载 | 专用环境 |
| 安全、准入控制与防护 | TC-WL-001 | 终端白名单与 USIM-IMEI 绑定 | 终端白名单与 USIM-IMEI 绑定 | 通用环境 |
| 安全、准入控制与防护 | TC-WL-002 | 基站白名单 | 基站白名单 | 通用环境 |
| 安全、准入控制与防护 | TC-SEC-001 | SUCI、鉴权与 NAS 安全 | SUCI、鉴权与 NAS 安全 | 通用环境 |
| 安全、准入控制与防护 | TC-SEC-002 | 违规外联检测、告警与阻断 | 违规外联检测、告警与阻断 | 通用环境 |
| 安全、准入控制与防护 | SE-GM-001 | 国密 SUCI | 国密 SUCI | 专用环境 |
| 安全、准入控制与防护 | SE-GM-002 | 国密 AKA 双向鉴权 | 国密 AKA 双向鉴权 | 专用环境 |
| 安全、准入控制与防护 | SE-GM-003 | ZUC/NAS/用户面安全 | ZUC/NAS/用户面安全 | 专用环境 |
| 安全、准入控制与防护 | SE-HSM-001 | 数据安全存储与密码机 | 数据安全存储与密码机 | 专用环境 |
| 安全、准入控制与防护 | SE-IPSEC-001 | 跨地域链路加密 | 跨地域链路加密 | 专用环境 |
| 安全、准入控制与防护 | SE-DEF-001 | 违规外联检测与阻断 | 违规外联检测与阻断 | 专用环境 |
| 安全、准入控制与防护 | SE-DEF-002 | 异常信令检测 | 异常信令检测 | 专用环境 |
| 安全、准入控制与防护 | SE-DEF-003 | 过载与 DDoS 防护 | 过载与 DDoS 防护 | 专用环境 |
| 物联网与特殊终端 | CC-REDCAP-001 | RedCap Indication、NR_REDCAP RAT、注册与移动性 | RedCap Indication、NR_REDCAP RAT、RedCap 注册、移动性注册更新、4G/5G 互操作 | 专用环境 |
| 物联网与特殊终端 | CC-REDCAP-002 | RedCap eDRX 节电功能 | RedCap eDRX | 专用环境 |
| 物联网与特殊终端 | CC-IOT-001 | LTE Cat.1 接入、移动性和路由优化 | LTE Cat.1 终端接入、移动性、数据路由优化 | 专用环境 |
| 物联网与特殊终端 | CC-IOT-002 | NB-IoT CP 模式、PSM/eDRX、速率控制 | NB-IoT CP 模式、PSM、eDRX、速率控制 | 专用环境 |
| 语音、IMS 与短信 | TC-IMS-001 | IMS DNN 与 P-CSCF 下发 | IMS DNN 与 P-CSCF 下发 | 通用环境 |
| 语音、IMS 与短信 | SE-IMS-001 | SIP 终端接入与互通 | SIP 终端接入与互通 | 专用环境 |
| 语音、IMS 与短信 | SE-IMS-002 | VoNR/VoLTE 与 P-CSCF 下发 | VoNR/VoLTE 与 P-CSCF 下发 | 专用环境 |
| 语音、IMS 与短信 | SE-IMS-003 | PSTN/运营商网络互通 | PSTN/运营商网络互通 | 专用环境 |
| 语音、IMS 与短信 | CC-IMS-001 | IMS 单节点集成形态 | 单节点集成 P-CSCF/I-CSCF/S-CSCF/UDM/SMSC 等网元 | 通用环境 |
| 语音、IMS 与短信 | CC-IMS-002 | IMS 2000 注册、500 路语音并发 | 单节点 2000 用户注册、500 路语音通话并发 | 专用环境 |
| 语音、IMS 与短信 | CC-IMS-003 | IMS UPDATE、业务触发、消息管理 | IMS 会话刷新 UPDATE、业务触发、消息管理 | 通用环境 |
| 语音、IMS 与短信 | CC-IMS-004 | SMSC 多媒体短信 | SMSC 支持图像、音频等复杂多媒体短信格式 | 专用环境 |
| 语音、IMS 与短信 | CC-IMS-005 | EPS Fallback/RAT Fallback 与语音域选择 | 语音域选择、EPS Fallback、RAT Fallback | 专用环境 |
| 语音、IMS 与短信 | CC-IMS-006 | IMS/SBC/MGW 主备冗余和负载均衡 | 语音主备冗余、负载均衡高可靠方案 | 专用环境 |
| 语音、IMS 与短信 | CC-IMS-007 | 呼叫前转到第三方网络 | 当自身网络内无法呼通终端时，将呼叫转移到第三方网络 | 专用环境 |
| 运维、部署、兼容性与数据同步 | TC-OAM-001 | 配置查询、导入、导出、同步与激活 | 配置查询、导入、导出、同步与激活 | 通用环境 |
| 运维、部署、兼容性与数据同步 | TC-OAM-002 | 日志、接口跟踪与用户跟踪 | 日志、接口跟踪与用户跟踪 | 通用环境 |
| 运维、部署、兼容性与数据同步 | TC-OAM-003 | 性能采集、补采、报表与门限 | 性能采集、补采、报表与门限 | 通用环境 |
| 运维、部署、兼容性与数据同步 | TC-DEP-001 | 一键部署与一键升级 | 一键部署与一键升级 | 通用环境 |
| 运维、部署、兼容性与数据同步 | SE-PERF-001 | 性能采集、补采、报表与门限 | 性能采集、补采、报表与门限 | 专用环境 |
| 运维、部署、兼容性与数据同步 | SE-PERF-002 | 容量与并发 | 容量与并发 | 专用环境 |
| 运维、部署、兼容性与数据同步 | CC-STD-001 | 3GPP 版本一致性和能力声明 | 基于 3GPP R16 稳定商用版本，同时验证手册采用的 Release 19/20 跟踪口径不会掩盖 R16 基础能力 | 通用环境 |
| 运维、部署、兼容性与数据同步 | CC-SBI-001 | SBA/SBI 全接口服务发现、鉴权和异常响应 | SBA 架构、SBI 接口、服务发现、异常响应 | 通用环境 |
| 运维、部署、兼容性与数据同步 | CC-FUSION-001 | 4G/5G 融合网元产品形态 | AMF/MME、UDM/HSS、PCF/PCRF、SMF/PGW-C、UPF/PGW-U 等融合网元声明 | 专用环境 |
| 运维、部署、兼容性与数据同步 | CC-COMP-001 | X86/ARM 服务器部署 | 支持 X86 或 ARM 服务器部署 | 通用环境 |
| 运维、部署、兼容性与数据同步 | CC-COMP-002 | 国产 OS、达梦数据库、自主 CPU | 银河麒麟 V10、达梦 V8.4、鲲鹏/飞腾/海光等国产化适配 | 专用环境 |
| 运维、部署、兼容性与数据同步 | CC-DEP-001 | 虚拟机部署兼容矩阵 | 支持虚拟机部署 | 通用环境 |
| 运维、部署、兼容性与数据同步 | CC-DEP-002 | 5GC/EPC 网元下沉和逐网元健康验收 | 5GC 全网元下沉部署、EPC 网元部署 | 通用环境 |
| 运维、部署、兼容性与数据同步 | CC-DEP-003 | 容器化 / Kubernetes 部署冒烟 | 容器化部署、Kubernetes 部署、滚动升级和服务恢复 | 专用环境 |
| 运维、部署、兼容性与数据同步 | CC-LMT-001 | 本地 LMT 页面全流程 | 本地 LMT 管理维护页面，网元配置、开户、状态、告警、安全策略查询 | 通用环境 |
| 运维、部署、兼容性与数据同步 | CC-LOG-001 | OMC 操作日志、安全事件日志保护 | 网元/OMC 操作日志管理、安全事件日志记录与保护 | 通用环境 |
| 运维、部署、兼容性与数据同步 | CC-UDM-001 | UDM 开户、APN/DNN 签约、速率设置 UI/API | UDM 开户、APN/DNN 签约、速率设置 | 通用环境 |
| 运维、部署、兼容性与数据同步 | CC-UDM-002 | 大网开户数据与本地专网数据同步 | 大网开户数据与本地专网开户数据同步 | 专用环境 |
...
**专用环境:** 两个支持 Xn 的 gNB 或 RAN 仿真仪，1 个可移动 UE，公司 5GC AMF/SMF/UPF，连续 N3/N6 抓包。
...
**本地自动化:** `内部自动化测试套件/5glan-l2-switch`
...
mongosh <产品数据库> < tests/5glan/add_subscribers.js
...
**前置条件:** 网管平台/API 网关、外部系统模拟器、合法 API 凭证。
...
**本地自动化:** `内部自动化测试套件/5glan-l2-l3-gateway`
...
- 网管平台 显示下挂设备 MAC、IP、所属 SUPI、DNN、首次发现时间、最后活跃时间。
- 设备离线后状态更新。
...
- NRF NFProfile、AMF UE 数、gNB SCTP 连接、网管平台 拓扑状态。
...
- SMF/网管平台 状态从 Active/Standby 更新为新的角色。
...
- 故障注入时间点、N3/N6 抓包、业务序列号丢包统计、UPF 角色日志、网管平台 告警。
...
1. 网管平台 添加 SUPI、IMEI/IMEISV、允许状态。
...
3. 在线 UE 从白名单删除。
4. 通过 网管平台/API 在运行时新增、禁用、恢复白名单条目，不重启 AMF。
...
1. 网管平台 配置允许的 gNB ID、TAI、PLMN、SCTP peer IP。
...
**本地自动化:** `内部自动化测试套件/regression-separated`
...
**规范依据:** SA5 管理规范族，项目 网管平台 API 设计  

**正常流程**

1. 网管平台 导出当前 AMF/SMF/UPF/用户/切片配置。
...
**专用环境:** 网管平台、Prometheus/Grafana 或等效性能平台，统一 NTP 时间。
...
1. 在国产 OS 上安装 5GC 和 网管平台。
2. 将用户、订阅、策略、网管平台 配置迁移到达梦数据库。
...
- 若无达梦数据库环境，可执行数据库适配层静态检查、SQL 迁移脚本语法检查和 产品数据库 基线回归，但结论只能记为 `WARN`。
...
1. 使用容器编排文件部署 NRF、AMF、SMF、UPF、UDM、AUSF、UDR、PCF、网管平台 和数据库。
...
**规范依据:** TS 28.532 management services，TS 28.541 management service model，TS 28.545 fault supervision，项目 LMT/网管平台 UI 规范。  
**前置条件:** LMT/网管平台 页面可访问，测试账号具备不同权限角色。
...
**前置条件:** 网管平台/OMC 日志模块、安全事件模块、审计账号。
...
**前置条件:** 网管平台/UDM API 可用。

**正常流程**

1. 通过 网管平台 新增用户 SUPI/K/OPc。
...
**前置条件:** 大网数据源模拟器、本地 UDM/网管平台、同步任务。
